...
/*   Updated: 2022/12/16 14:02:21 by kczichow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx_put_pixel(g_img, xc+x , yc+y, get_rgba(0, 0, 255 , 0));
	mlx_put_pixel(g_img, xc-x , yc+y, get_rgba(0, 0, 255 , 0));
    mlx_put_pixel(g_img, xc+x , yc-y, get_rgba(0, 0, 255 , 0));
    mlx_put_pixel(g_img, xc-x , yc-y, get_rgba(0, 0, 255 , 0));
    mlx_put_pixel(g_img, xc+y , yc+x, get_rgba(0, 0, 255 , 0));
    mlx_put_pixel(g_img, xc-y , yc+x, get_rgba(0, 0, 255 , 0));
    mlx_put_pixel(g_img, xc+y , yc-x, get_rgba(0, 0, 255 , 0));
    mlx_put_pixel(g_img, xc-y , yc-y, get_rgba(0, 0, 255 , 0));
}
...
{	
	...
	while(x > (i - 10))
	{
		mlx_put_pixel(g_img, x , y, get_rgba(0, 0, 255 , 0));
		x--;
	}
	while(y > (j - 10))
	{
		mlx_put_pixel(g_img, x , y, get_rgba(20, 20, 0 , 0));
		y--;
	}
	printf("x is: %d\n", x);
	printf("y is: %d\n", x);
...
void drawcircle(mlx_image_t	*g_img, mlx_t *mlx)
{
    ...
    while (x >= y)
    {
        mlx_put_pixel(g_img, x0 + x , y0 + y, get_rgba(0, 0, 255 , 0));
        mlx_put_pixel(g_img, x0 + y , y0 + x, get_rgba(0, 0, 255 , 0));
        mlx_put_pixel(g_img, x0 - y , y0 + x, get_rgba(0, 0, 255 , 0));
        mlx_put_pixel(g_img, x0 - x , y0 + y, get_rgba(0, 0, 255 , 0));
        mlx_put_pixel(g_img, x0 - x , y0 - y, get_rgba(0, 0, 255 , 0));
        mlx_put_pixel(g_img, x0 - y , y0 - x, get_rgba(0, 0, 255 , 0));
